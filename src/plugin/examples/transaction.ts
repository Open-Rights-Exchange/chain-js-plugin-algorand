...
async function run() {
  ...
  const { feeStringified } = await transaction.getSuggestedFee(Models.TxExecutionPriority.Average)
  console.log('suggestedFee: ', feeStringified)
  ...
  try {
    ...
    console.log('actual fee: ', transaction.actualCost)
  } catch (err) {
    console.log(err)
  }
}
